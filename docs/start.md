...
Next, open a terminal in the engine folder of the project,and run the following command to download the remaining third party softwares: `git submodule update --init --recursive`
...
```
And repeat for all four uniforms in the sky shader.
```cpp
    size_t id1 = Renderer::RegisterUniform(program, "u_skyColorDay", UniformType::UNIFORM_VEC4);
    size_t id2 = Renderer::RegisterUniform(program, "u_skyColorNight", UniformType::UNIFORM_VEC4);
    size_t id3 = Renderer::RegisterUniform(program, "u_sunColorDay", UniformType::UNIFORM_VEC4);
    size_t id4 = Renderer::RegisterUniform(program, "u_sunColorRise", UniformType::UNIFORM_VEC4);
...
We use CMake to build Syngine & Syngine games. In a terminal instance in the `build` directory of the project, run the following command to configure the compiler: `cmake .. -G Ninja` If you use another build system than Ninja, feel free to swap the generator. In addition, Debug mode may be turned on by adding `-DCMAKE_BUILD_TYPE:STRING=Debug` to the command. 
...
After it is built, you can run the executable or app bundle found in either the `build/debug` or `build/release` folder, depending on release mode. (Or it could just be in `build/`) If all is good, the game should launch immediately and without presenting any errors, and the window should be pure blue, as that is the sky shader default color.
