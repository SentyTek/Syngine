...
std::filesystem::path _GetAppDataPath(const std::string& appName) {
    ...
    if (appName.empty()) {
        throw std::invalid_argument(
            "App name cannot be empty when getting AppData path.");
    }
#ifdef _WIN32
    ...
    if (status != noErr) {
        throw std::runtime_error("Error finding Application Support folder: " +
    ...
    // Compose the Appdata folder path: ~/Library/Application
    // Support/SentyTek/<appName>
    appDataFolder = std::filesystem::path(path) / "SentyTek" / appName;
    ...
    if (!homeDir) {
        ...
        homePath = std::string(homeDir) +
                   "/.local/share"; // Default fallback to ~/.local/share
    }

    // Compose the AppData folder path: $XDG_DATA_HOME/SentyTek/<appName> (or
    // ~/.local/share/SentyTek/<appName>)
    ...
    // In addition, tell the OS to make the folder if it doesn't exist, since
    // we'll be writing files there.
    try {
        std::filesystem::create_directories(appDataFolder);
    } catch (const std::filesystem::filesystem_error& e) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "Failed to create AppData directory: %s",
                     e.what());
    }
