...
namespace Syngine {
namespace {
float CapsuleCenterOffset(float height) { return height * 0.5f; }
} // namespace
...
PlayerComponent::PlayerComponent(const PlayerComponent& other)
    ...
    if (this->m_transform) {
        JPH::Ref<JPH::CharacterSettings> settings =
        ...
        settings->mShape = new JPH::CapsuleShape(
            standHeight / 2, playerRadius); // Half height and radius
        settings->mFriction = 0.45f;
        settings->mMass     = 80.0f;

        Vector3    pos = m_transform->GetPosition();
        JPH::RVec3 initialPosition(
            pos.x(), pos.y() + CapsuleCenterOffset(standHeight), pos.z());
        this->m_character =
        ...
        this->m_camera->SetPosition(
            Math::Vector3(pos.x(), pos.y() + m_eyeHeight, pos.z()));
    }
}
...
void PlayerComponent::Init(Syngine::CameraComponent* camera) {
    ...
    settings->mLayer = Syngine::Layers::MOVING; // Set to the moving layer
    settings->mShape = new JPH::CapsuleShape(
        halfHeight, playerRadius); // Half height and radius
    settings->mFriction = 0.45f;
    settings->mMass     = 80.0f;
...
void PlayerComponent::_HandleInput(const SDL_Event& event) {
    ...
    if (m_simulate) {
        if (event.type == SDL_EVENT_MOUSE_MOTION) {
            ...
            m_currentPitch = bx::clamp(m_currentPitch,
                                       -bx::toRad(maxPitchAngle),
                                       bx::toRad(maxPitchAngle));
            ...
            if (event.key.key == SDLK_LCTRL) {
                if (m_playerState == PlayerState::SLIDING) {
                    m_playerState = PlayerState::WALKING; // Stop sliding when
                                                          // releasing crouch
                } else if (m_playerState == PlayerState::CROUCHING) {
                    m_playerState = PlayerState::IDLE; // Stand up immediately
                                                       // on crouch release
                }
            }
        }
...
void PlayerComponent::Update(float deltaTime) {
    ...
    // Local variables to allow for smooth transitions without affecting the
    // actual target values immediately
    ...
    if (m_simulate) {
        ...
        if (enableSliding && keystate[SDL_SCANCODE_LSHIFT] &&
            keystate[SDL_SCANCODE_LCTRL] && isGrounded &&
            m_prevPlayerState == PlayerState::SPRINTING) {
            m_playerState = PlayerState::SLIDING;
            m_targetMoveSpeed *=
                sprintMult * slideSpeedMult; // Increase speed while sliding
            // m_targetEyeHeight = 0.7f;
        ...
        if (m_playerState == PlayerState::CROUCHING ||
            ...
            if (m_prevPlayerState != PlayerState::CROUCHING &&
                ...
                m_character->SetShape(
                    ...
                    0.1f); // Half height and radius

                Vector3 feetPos = m_transform->GetPosition();
                m_character->SetPosition(
                    JPH::RVec3(feetPos.x(),
                               feetPos.y() + CapsuleCenterOffset(crouchHeight),
                               feetPos.z()));
            }
            l_targetEyeHeight =
            ...
            if (m_prevPlayerState == PlayerState::CROUCHING ||
                ...
                m_character->SetPosition(
                    JPH::RVec3(feetPos.x(),
                               feetPos.y() + CapsuleCenterOffset(standHeight),
                               feetPos.z()));
        ...
        if (m_character) {
            ...
            if (m_moveDirection.length() > 0.001f) {
                ...
                desiredHorizontalVel.SetX(m_moveDirection.x() *
                                          m_realMoveSpeed);
                desiredHorizontalVel.SetZ(m_moveDirection.z() *
                                          m_realMoveSpeed);
                if (m_playerState != PlayerState::SLIDING &&
                    m_playerState != PlayerState::CROUCHING &&
                    m_playerState != PlayerState::SPRINTING) {
            ...
            // Get current velocity and preserve the vertical component (for
            // gravity)
    ...
    m_camera->SetFOV(bx::lerp(m_camera->GetFOV(),
                              l_targetFov,
                              1.0f - bx::exp(-fovLerpSpeed * m_deltaTime)));
    m_eyeHeight = bx::lerp(m_eyeHeight,
                           l_targetEyeHeight,
                           1.0f - bx::exp(-eyeHeightLerpSpeed * m_deltaTime));
    m_realMoveSpeed =
        bx::lerp(m_realMoveSpeed,
                 m_targetMoveSpeed,
                 1.0f - bx::exp(-moveSpeedLerpSpeed * m_deltaTime));
}
...
void PlayerComponent::PostPhysicsUpdate() {
    ...
    if (m_character) {
        const float maxSeparationDistance =
            0.1f; // Distance to consider "grounded"
    ...
    const float colliderHeight = (m_playerState == PlayerState::CROUCHING ||
                                  m_playerState == PlayerState::SLIDING)
                                     ? crouchHeight
                                     : standHeight;
    ...
    bx::Vec3    newPos =
        bx::lerp(currentPos,
                 targetPos,
                 1.0f - bx::exp(-positionLerpSpeed * m_deltaTime));
    m_transform->SetWorldPosition(Vector3(newPos));

    // Update camera position and orientation
    m_camera->SetPosition(
        Math::Vector3(newPos.x, newPos.y + m_eyeHeight, newPos.z));
