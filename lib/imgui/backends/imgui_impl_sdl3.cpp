...
//  ImGuiPlatformIO interface. 2026-03-25: [Docking] Use
//  SDL_HAS_EVENT_DISPLAY_USABLE_BOUNDS_CHANGED (SDL 3.4.0+) when available to
//  avoid refreshing monitor work area every frame on Windows. (#8415)
//  2026-03-09: [Docking] Fixed an issue dated 2025/04/09 (1.92 WIP) where a
//  refactor+merge caused ImGuiBackendFlags_HasMouseHoveredViewport to never be
//  set, causing foreign windows to be ignored when deciding of hovered
//  viewport. (#9284) 2026-02-13: Inputs: systems other than X11 are back to
//  starting mouse capture on mouse down (reverts 2025-02-26 change). Only X11
//  requires waiting for a drag by default (not ideal, but a better default for
//  X11 users). Added ImGui_ImplSDL3_SetMouseCaptureMode() for X11 debugger
//  users. (#3650, #6410, #9235) 2026-01-15: Changed GetClipboardText() handler
//  to return nullptr on error aka clipboard contents is not text. Consistent
//  with other backends. (#9168) 2025-11-05: Fixed an issue with missing
//  characters events when an already active text field changes viewports.
//  (#9054) 2025-10-22: Fixed Platform_OpenInShellFn() return value (unused in
//  core). 2025-09-24: Skip using the SDL_GetGlobalMouseState() state when one
//  of our window is hovered, as the SDL_EVENT_MOUSE_MOTION data is reliable.
//  Fix macOS notch mouse coordinates issue in fullscreen mode + better perf on
//  X11. (#7919, #7786) 2025-09-18: Call platform_io.ClearPlatformHandlers() on
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
    ...
    case SDL_EVENT_KEY_UP: {
        ...
        // IMGUI_DEBUG_LOG("SDL_EVENT_KEY_%s : key=0x%08X ('%s'), scancode=%d
        ...
        //     event->key.mod, event->key.windowID, viewport ? viewport->ID :
        //     0);
    ...
    case SDL_EVENT_WINDOW_FOCUS_LOST: {
        ...
        // "SDL_WINDOWEVENT_FOCUS_LOST", event->window.windowID, viewport ?
        // viewport->ID : 0);
...
static void ImGui_ImplSDL3_UpdateMouseData() {
    ...
    if (bd->MouseCaptureMode == ImGui_ImplSDL3_MouseCaptureMode_Enabled) {
        ...
        for (int button_n = 0;
             button_n < ImGuiMouseButton_COUNT && !want_capture;
             button_n++)
...
static void ImGui_ImplSDL3_CreateWindow(ImGuiViewport* viewport) {
    ...
    sdl_flags |=
        use_opengl ? SDL_WINDOW_OPENGL : (bd->IsVulkan ? SDL_WINDOW_VULKAN : 0);
