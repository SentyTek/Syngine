  ...
  public:
    ...
    static inline scl::stream _ReadFromBundle(const std::string& bundlePath,
        ...
        try {
            ...
            if (!wts || !wts->stream()) {
                Logger::LogF(LogLevel::ERR, "Failed to open asset in bundle: %s. Does the asset exist?", assetPath.c_str());
                pack.close();
                return scl::stream();
            }
            ...
            Logger::LogF(LogLevel::ERR, "Error reading from bundle: %s", e.what());
            return scl::stream();
        }
    }
