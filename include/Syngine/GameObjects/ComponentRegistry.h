...
class ComponentRegistry {
  public:
    ...
    static void Register(Syngine::ComponentTypeID type,
                         ...
                         RegisterLuaFn            registerLuaFn = nullptr);
  ...
  private:
    ...
    static void _RegisterAllLuaBindings(
        sol::state& lua); // IMPORTANT: LuaManager calls this after Lua state is
                          // initialized
    friend class LuaManager; // Allow LuaManager to call _RegisterAllLuaBindings
};

// Helper struct to register components in the ComponentRegistry at static
// initialization time
struct ComponentRegistrar {
    ComponentRegistrar(
        Syngine::ComponentTypeID         type,
        ...
        ComponentRegistry::Register(type,
                                    std::move(parseFn),
                                    std::move(instantiateFn),
                                    std::move(registerLuaFn));
    }
};
