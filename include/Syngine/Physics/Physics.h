...
class BPLayerInterfaceImpl : public BroadPhaseLayerInterface {
  public:
    ...
    GetBroadPhaseLayerName(BroadPhaseLayer inLayer) const override {
        ...
        default: JPH_ASSERT(false, "Invalid layer"); return "INVALID";
        }
    }
...
class ObjectVsBroadPhaseLayerFilterImpl : public ObjectVsBroadPhaseLayerFilter {
  public:
    virtual bool ShouldCollide(ObjectLayer     inLayer1,
        ...
        case Layers::MOVING:
            return true; // Moving objects can collide with both moving and
                         // non-moving objects
        default: JPH_ASSERT(false, "Invalid layer"); return false;
        }
    }
};
...
class ObjectLayerPairFilterImpl : public ObjectLayerPairFilter {
  public:
    virtual bool ShouldCollide(ObjectLayer inLayer1,
                               ObjectLayer inLayer2) const override {
        switch (inLayer1) {
...
class SynContactListener : public JPH::ContactListener {
  public:
    ...
    OnContactValidate(const Body&               inBody1,
                      const Body&               inBody2,
                      RVec3Arg                  inBaseOffset,
                      const CollideShapeResult& inResult) override {
        // This is called when a contact is created. Can be used to filter out
        // contacts. For example, check if the bodies are in the same layer and
    ...
    virtual void OnContactAdded(const Body&            inBody1,
                                ...
                                ContactSettings&       inSettings) override {
        // This is called when a contact is added. Can be used to modify the
        // contact settings. For example, change the friction or restitution of
        // the contact. SDL_Log("Contact added between %u and %u",
    ...
    virtual void OnContactPersisted(const Body&            inBody1,
        ...
        // This is called when a contact is persisted. For example, to update
        // the contact settings. SDL_Log("Contact persisted between %u and %u",
        // inBody1.GetID(), inBody2.GetID());
    }

    virtual void
    OnContactRemoved(const SubShapeIDPair& inSubShapePair) override {
        // This is called when a contact is removed. For example, to clean up
        // any resources allocated for the contact. SDL_Log("Contact removed");
    }
};
...
class Phys {
  public:
    ...
    /// @since v0.0.1
    /// @internal
    const ObjectVsBroadPhaseLayerFilterImpl&
    _GetObjectVsBroadPhaseLayerFilter() const {
